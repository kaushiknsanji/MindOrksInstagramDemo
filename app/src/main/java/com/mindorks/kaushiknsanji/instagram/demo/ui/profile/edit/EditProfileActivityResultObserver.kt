...
/**
 * [EditProfileActivity] Lifecycle observer class to receive and handle the activity result in a separate
 ...
 * @property viewModel [EditProfileViewModel] instance which is the Primary ViewModel of the [EditProfileActivity]
 ...
 *
 * @constructor Creates an instance of [EditProfileActivityResultObserver] for registering calls, storing
 * and handling their callbacks in a separate class.
 *
 * @author Kaushik N Sanji
 */
class EditProfileActivityResultObserver(
    registry: ActivityResultRegistry,
    private val viewModel: EditProfileViewModel,
    private val camera: Camera
) : BaseActivityResultObserver<EditProfileActivity>(registry) {
     ...
     * [androidx.activity.result.ActivityResultLauncher]s for the [EditProfileActivity].
     *
     ...
     * @param owner [LifecycleOwner] of the [EditProfileActivity] that makes the call.
     * @param activity [EditProfileActivity] instance derived from [owner].
     */
    override fun initResultLaunchers(
        ...
        galleryLauncher = galleryAppLauncher(registry, owner, activity)
    ...
    private fun galleryAppLauncher(
        ...
        reqKeyGenerator(TAG, PhotoOptionsDialogFragment.REQUEST_IMAGE_PICK),
        owner,
        ...
        try {
            result?.let { uri: Uri ->
                ...
                ?: activity.showMessage(R.string.error_retry) // Ask to retry when we do not have the URI to the Image picked
        } catch (e: FileNotFoundException) {
            e.printStackTrace()
            // Ask to retry in case of failure while opening the input stream to the URI
            activity.showMessage(R.string.error_retry)
        }
    }
    ...
    companion object {
        ...
        const val TAG = "EditProfileActivityResultObserver"
    }
}